...
def hci_write(dat):
    ...
    #cyw_write_backplane_reg_u32(wifi_base + SEND_TAIL, send_tail + len(dat))
    #data_send_toggle()
...
print("SEND x03x00x00x01x03x0cx00x00")
hci_write(b'\x01\x03\x0c\x00')
dat = hci_read()
print_hex('Received', dat)

print("SEND x03x00x00x01x01x10x00x00")
hci_write(b'\x01\x01\x10\x00')
dat = hci_read()
print_hex('Received', dat)

print("SEND x05x00x00x01x0cx20x02x00x00")
...
print("SEND x05x00x00x01x0cx20x02x00x00x00x00x00")
hci_write(b'\x01\x0c\x20\x02\x00\x00')
dat = hci_read()
print_hex('Received', dat)
